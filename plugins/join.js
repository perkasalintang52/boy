...
let handler = async (m, { conn, text, isOwner, groupMetadata }) => {
    ...
    if (global.db.data.users[m.sender].joinlimit == 9999) return m.reply('Maaf kamu sudah tidak bisa menggunakan free join..\nHarap hubungi *owner* kami')
    global.db.data.users[m.sender].joinlimit -= 9999
   // let id = m.chat
